...
pub mod app {
    ...
    use esp32c3_hal::{
        clock::ClockControl, peripherals::{self, Peripherals},
        prelude::*, timer::TimerGroup,
    ...
    fn init(_: init::Context) -> (Shared, Local) {
        ...
        ::rtt_target::print_impl::write_str(0, "init done\n");
        interrupt::enable(
    ...
    fn idle(_: idle::Context) -> ! {
        ...
        ::rtt_target::print_impl::write_str(0, "hello from idle\n");
        loop {}
    }
    /// Execution context
    #[allow(non_snake_case)]
    ...
    pub fn __rtic_internal_foo_spawn() -> Result<(), ()> {
        unsafe {
            if __rtic_internal_foo_EXEC.try_allocate() {
                ...
                rtic::pend(esp32c3::Interrupt::FROM_CPU_INTR1);
                Ok(())
            } else {
                Err(())
            }
            ...
            || {
                __rtic_internal_foo_EXEC
                    .poll(|| {
                        ...
                        ::rtt_target::print_impl::write_str(0, "pend should be in\n");
                        rtic::pend(esp32c3::Interrupt::FROM_CPU_INTR1);
                    });
            },
        );
